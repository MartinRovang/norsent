...
def chart5():
    try:
        ...
        conn = sql.connect("twitter.db")
        df = pd.read_sql("SELECT * FROM users WHERE (sent LIKE '%Kristelig Folkeparti%' OR sent LIKE '%Knut Arild Hareide%') ",conn)
        Y = movingavarage(floatify(df['what'].values)[-500:],25)
        labels = np.linspace(len(Y),0,len(Y))
        return render_template('chart.html' ,sent = Y ,labels = labels, name = name)
    except Exception as e:
...
def create_changes_response_party():
    ...
    KRFDf = pd.read_sql("SELECT * FROM users WHERE (sent LIKE '%Kristelig Folkeparti%' OR sent LIKE '%Knut Arild Hareide%') ", conn)
